...
class _HomepageState extends State<Homepage> with HomeComponents, MyComponents{
  ...
  void initState() {
    ...
    initParam(context).whenComplete(() { setState(() {
      currentLocale = L10n.locals[0]; // Initialize with the first locale
      GeneralStreams.languageStream.stream.listen((locale) => setState(() { currentLocale = locale; }) );
          ...
          return PopScope(
            ...
            child: launchLoading ? MyWidgets.MyLoading2(context, isDarkMode) : GestureDetector( onTap: () => FocusManager.instance.primaryFocus?.unfocus(), child: Scaffold(
              ...
              drawer: HomeWidgets.MyDrawer(context, isDarkMode, appVersion: appVersion, domainName: domainName, 
                items: [
                  HomeWidgets.Item(context, icon: FontAwesomeIcons.userAlt, label: Localizations.profile,
                    onTap: () => myPushNamed( context, setState, ProfilePage.routeName , arguments: MyArguments(token, prevPath: "/home", user: jsonEncode(user), outlets: jsonEncode(outlets)))),
          
                  HomeWidgets.Item(context, icon: FontAwesomeIcons.storeAlt, label: Localizations.manage_outlet,
                  ...
                  HomeWidgets.Item(context, icon: FontAwesomeIcons.history, label: Localizations.track_history,
                    onTap: () => myPushNamed( context, setState, TrackingHistoryPage.routeName, arguments: MyArguments(token, prevPath: "/home"))),
                  HomeWidgets.Item(context, icon: FontAwesomeIcons.images, label: Localizations.image_status,
                    onTap: () => myPushNamed( context, setState, ImageStatusPage.routeName , arguments: MyArguments(token, prevPath: "/home", username: user['id'] ))),
          
                  Divider(color: Theme.of(context).colorScheme.onTertiary.withOpacity(0.5),),
          
                  HomeWidgets.Item(context, icon: FontAwesomeIcons.gear, label: Localizations.settings,
                    onTap: () => myPushNamed( context, setState, SettingsPage.routeName , arguments: MyArguments(token, prevPath: "/home"))),
                  HomeWidgets.Item(context, icon: FontAwesomeIcons.signOut, label: Localizations.log_out,
                    onTap: () async {
